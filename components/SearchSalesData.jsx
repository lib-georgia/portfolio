...
const SearchSalesData = (props) => {
    const { searchSalesProduct } = props
            ...
            const searchTotalPrice = Object.keys(searchSalesProduct).reduce((sum, key) => sum + parseInt(searchSalesProduct[key].price || 0) , 0);
            const searchHamburgerTotalPrice = Object.keys(searchHamburger).reduce((sum, key) => sum + parseInt(searchHamburger[key].price || 0), 0);
            const searchSetMenuTotalPrice = Object.keys(searchSetMenu).reduce((sum, key) => sum + parseInt(searchSetMenu[key].price || 0), 0);
            const searchSideMenuTotalPrice = Object.keys(searchSideMenu).reduce((sum, key) => sum + parseInt(searchSideMenu[key].price || 0), 0);
    const searchDrinkTotalPrice = Object.keys(searchDrink).reduce((sum, key) => sum + parseInt(searchDrink[key].price || 0), 0);
    ...
    const searchHamburgerRatio = Math.round(searchHamburgerTotalPrice / searchTotalPrice * 100) + '%'
        ...
        <>
            {(() => {
                if (searchSalesProduct) {
                    return <>
                        <ShowSalesData title={'合計'} salesData={searchAllProductSalesDataSort} data={searchCircleAllProductData} option={searchAllProductOption} onClick={() => { setShowAllSalesData(true); setShowAllProduct(true) }} />
                        <ShowSalesData title={'ハンバーガー'} salesData={searchHamburgerSalesDataSort} data={searchCircleHamBurgerData} option={searchHamburgerOption} onClick={() => { setShowAllSalesData(true);setShowHamburger(true) }} />
                        <ShowSalesData title={'セットメニュー'} salesData={searchSetMenuSalesDataSort} data={searchCircleSetMenuData} option={searchSetMenuOption} onClick={() => { setShowAllSalesData(true);setShowSetMenu(true) }} />
                        <ShowSalesData title={'サイドメニュー'} salesData={searchSideMenuSalesDataSort} data={searchCircleSideMenuData} option={searchSideMenuOption} onClick={() => { setShowAllSalesData(true);setShowSideMenu(true) }} />
                        <ShowSalesData title={'ドリンク'} salesData={searchDrinkSalesDataSort} data={searchCircleDrinkData} option={searchDrinkOption} onClick={() => { setShowAllSalesData(true);setShowDrink(true) }} />
                    </>
                } else {
                    return <></>
                }
